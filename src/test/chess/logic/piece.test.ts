...
describe('canMoveFrom', () => {
  it('returns true if there is at least one destination', () => {
    ...
    let piece = { id: 21, player_number: 1, type: 'pawn', has_moved: false, selected: false };
    let from = { id: 'e4', x: 4, y: 4, piece: { id: 21, player_number: 1, type: 'pawn', has_moved: false, selected: false } };
  ...
  it('returns false if there is no destinations', () => {
    ...
    let piece = { id: 32, player_number: 1, type: 'rook', has_moved: false, selected: false };
...
describe('destinations', () => {
  describe('when pawn', () => {
    it('returns squares diagonal capture squares, forward move squares and', () => {
      let gameState = pawnCaptureGameState();
      let pawn = { id: 21, player_number: 1, type: 'pawn', has_moved: false, selected: false };
      let square = { id: 'e4', x: 4, y: 4, piece: pawn };
      let expected = [
        { id: 'e5', x: 4, y: 3, piece: null },
        { id: 'd5', x: 3, y: 3, piece: { id: 12, player_number: 2, type: 'pawn', has_moved: false, selected: false } }
      ];
  ...
  describe('when rook', () => {
    it('returns orthogonal squares', () => {
      ...
      let rook =  { id: 32, player_number: 1, type: 'rook', has_moved: false, selected: false };
      let square = { id: 'h2', x: 7, y: 6, piece: { id: 32, player_number: 1, type: 'rook', has_moved: false, selected: false } };
      let expected = [
  ...
  describe('when knight', () => {
    it('must return 2 away and 1 across squares', () => {
      ...
      let knight = { id: 31, player_number: 1, type: 'knight', has_moved: false, selected: false };
      let square = { id: 'g1', x: 6, y: 7, piece: { id: 31, player_number: 1, type: 'knight', has_moved: false, selected: false } };
      let expected = [
        { id: 'f3', x: 5, y: 5, piece: null },
        { id: 'h3', x: 7, y: 5, piece: { id: 16, player_number: 2, type: 'pawn', has_moved: false, selected: false } },
        { id: 'e2', x: 4, y: 6, piece: null }
      ];
  ...
  describe('when bishop', () => {
    it('returns diagonal squares', () => {
      ...
      let bishop = { id: 30, player_number: 1, type: 'bishop', has_moved: false, selected: false };
      let square = { id: 'f1', x: 5, y: 7, piece: bishop };
      let expected = [
        { id: 'd3', x: 3, y: 5, piece: { id: 12, player_number: 2, type: 'pawn', has_moved: false, selected: false } },
        { id: 'h3', x: 7, y: 5, piece: null },
        { id: 'e2', x: 4, y: 6, piece: null },
        { id: 'g2', x: 6, y: 6, piece: null }
      ];
  ...
  describe('when queen', () => {
    it('returns orthogonal and diagonal squares', () => {
      ...
      let queen = { id: 28, player_number: 1, type: 'queen', has_moved: false, selected: false };
  ...
  describe('when king', () => {
    it('returns all the adjacent squares', () => {
      ...
      let king = { id: 29, player_number: 1, type: 'king', has_moved: false, selected: false };
      let square = { id: 'e1', x: 4, y: 7, piece: king };
      let expected = [
        { id: 'd2', x: 3, y: 6, piece: null },
        { id: 'e2', x: 4, y: 6, piece: { id: 13, player_number: 2, type: 'pawn', has_moved: false, selected: false } },
        { id: 'f2', x: 5, y: 6, piece: null },
...
describe('captureSquares', () => {
  ...
  describe('when king', () => {
    it('returns base destinations', () => {
      let gameState = kingMoveGameState();
      let king = { id: 29, player_number: 1, type: 'king', has_moved: false, selected: false };
      let square = { id: 'e1', x: 4, y: 7, piece: { id: 29, player_number: 1, type: 'king', has_moved: false, selected: false } };
      let expected = [
        { id: 'd2', x: 3, y: 6, piece: null },
        { id: 'e2', x: 4, y: 6, piece: { id: 13, player_number: 2, type: 'pawn', has_moved: false, selected: false } },
        { id: 'f2', x: 5, y: 6, piece: null },
    ...
    describe('and an pawn has moved 2 squares', () => {
      ...
      describe('and the pawn is not adjacent to the opposing pawn', () => {
        it('returns undefined', () => {
          let gameState = enPassantGameState();
          let pawn = { id: 19, player_number: 1, type: 'pawn', has_moved: false, selected: false };
...
describe('hasNotMoved', () => {
  it('must return true if it has not moved', () => {
    let piece = { id: 0, player_number: 1, type: 'pawn', has_moved: false, selected: false }; 
    expect(hasNotMoved(piece)).toBe(true);
  });

  it('must return false if it has moved', () => {
    let piece = { id: 0, player_number: 1, type: 'pawn', has_moved: true, selected: false }; 
...
describe('opponent', () => {
  ...
  it('must return 1 if player number is 2', () => {
    let piece = { id: 0, player_number: 2, type: 'pawn', has_moved: true, selected: false }; 
...
describe('deselect', () => {
  it('unmarks the piece as selected', () => {
    let piece = { id: 0, player_number: 2, type: 'pawn', has_moved: true, selected: true }; 
    deselect(piece);
...
describe('pawnMoveableDistance', () => {
  it('returns 2 if pawn is on starting rank', () => {
    let piece = { id: 0, player_number: 1, type: 'pawn', has_moved: true, selected: true }; 
...
describe('kingBaseDestinations', () => {
  it('returns squares that a king could normally move to', () => {
    let gameState = kingMoveGameState();
    let king = { id: 32, player_number: 1, type: 'rook', has_moved: false, selected: false };
    let square = { id: 'e1', x: 4, y: 7, piece: king };
...
describe('kingCastle', () => {
  ...
  it('returns empty if king has moved', () => {
    ...
    let king = { id: 32, player_number: 1, type: 'king', has_moved: true, selected: false };
    let square = { id: 'e1', x: 4, y: 7, piece: king };
    let expected: Array<Square> = [ ];
    let result = kingCastle(king, square, gameState);
    expect(result).toEqual(expected);
