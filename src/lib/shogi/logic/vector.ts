...
export const absDx = function(from: Point, to: Point): number {
  return Math.abs(dx(from, to));
};

export const absDy = function(from: Point, to: Point): number {
  return Math.abs(dy(from, to));
};
