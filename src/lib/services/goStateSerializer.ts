...
const goStateSerializer = function(state: GameState): string {
  ...
  let blackStones = state.points.filter(function(p) { return p.stone !== null && p.stone.playerNumber === 1; }).map(function(p) { return `[${ID_TO_POS[p.id]}]`; }).join('');
  let whiteStones = state.points.filter(function(p) { return p.stone !== null && p.stone.playerNumber === 2; }).map(function(p) { return `[${ID_TO_POS[p.id]}]`; }).join('');
